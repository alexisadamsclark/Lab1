...
plot 1 <- ggplot(data2, aes(x=fct_reorder(screen_name, n), y = n)) +
  geom_col() +
  coord_flip()
```
...
#Reproduce graph
plot2 <- ggplot(data4, aes(x=month, y=n)) + 
       geom_line() +
       geom_smooth()
```
...
plotnew_2 <- ggplot(data4, aes(x = month, y = n)) +
  geom_line(color = "gray40", size = 1) +
  geom_smooth(color = "magenta") +
  geom_area(alpha = 0.3, fill = "cornflowerblue") +
  labs(x = "Year (data summarized by month)", y = "Number of #rstats tweets", title = "Growth of the #rstats hashtag on twitter over time") +
